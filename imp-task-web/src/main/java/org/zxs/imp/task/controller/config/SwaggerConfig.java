package org.zxs.imp.task.controller.config;
...
@EnableWebMvc
@ComponentScan(basePackages = {"org.zxs.imp.task.controller"})  //需要扫描的包路径
public class SwaggerConfig extends WebMvcConfigurationSupport {
	...
	public Docket swaggerSpringMvcPlugin() {
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).groupName("business-api").select() // 选择那些路径和api会生成document
				.apis(RequestHandlerSelectors.basePackage("org.zxs.imp.task")).paths(paths())
	...
	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("领导人联系项目后端接口 API")
				.termsOfServiceUrl("http://blog.csdn.net/yangshijin1988").description("此API提供接口调用")
				.license("License Version 2.0").licenseUrl("http://blog.csdn.net/yangshijin1988").version("2.0")
				.build();
	}
}
